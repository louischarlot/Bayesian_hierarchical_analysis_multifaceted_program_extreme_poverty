...
# Title: Model 2 (See master thesis for more details)

# This is our code for Model 2. 
...
# We create a function that remove rows for which a specified column has NAs:
...
Covariates_X <- dplyr::select(data_1, X_intercept, treatment) 
...
fit_Cholesky_Intercept_no_Baseline <- stan(
  ...
  refresh = max(2000/10, 1),  # actualization of progress shown
  control=list(adapt_delta=0.99, max_treedepth = 14) # We reduce step size.
)



print(fit_Cholesky_Intercept_no_Baseline, pars = c("beta[2,1]", "beta[2,2]", "beta[2,3]", "beta[2,4]", "beta[2,5]", "beta[2,6]"))

plot(fit_Cholesky_Intercept_no_Baseline,
     pars = c("beta[2,1]", "beta[2,2]", "beta[2,3]", "beta[2,4]", "beta[2,5]", "beta[2,6]"), outer_level = 0.95, ci_level = 0.5)

# To launch shinystan:
#launch_shinystan(fit_Cholesky_Intercept_no_Baseline)
...
posterior_Intercept_no_Baseline <- as.matrix(fit_Cholesky_Intercept_no_Baseline)
plot_title <- ggtitle("",expression(atop(paste("Model 2 with priors ", gamma[k] %~% N(0,5), ", ", theta[k] %~% halfCauchy(0,2.5)),
...
plot_post_distrib_Intercept_no_Baseline <- mcmc_areas(posterior_Intercept_no_Baseline,
  ...
  xlim(-0.1, 1)
ggplot2::ggsave("plot_Model_2_Cholesky_Intercept_no_Baseline.png", width = 6, height = 6)
plot_post_distrib_Intercept_no_Baseline
